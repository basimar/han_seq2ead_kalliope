...
my $writer = XML::Writer->new(OUTPUT => $output, NEWLINES => 1, ENCODING => "utf-8", NAMESPACES => 1, PREFIX_MAP => {$xlink => 'xlink', $xsi => 'xsi', $xsischema => 'xsi:schemaLocation', $xmlns => ''});

my (%title, %alttitle, %alttitleintro, %ausreifung, %music, %date, %datenorm, %datenormhuman, %date008, %date008human, %size, %dimension, %carrier, %arrangement, %restrict, %content, %contentform, %bibliography, %level, %link, %note, %noteA, %acqinfo, %relatedmat, %bioghist, %findaid, %custodhist, %einband, %literatur, %institution, %institutionname, %signature, %signaturealt, %signatureold, %isilsysnum, %isilnum, %languages, %langcodes, %controlper, %controlpernorm, %controlpergnd, %controlperfamily, %controlkorp, %controlkorpnorm, %controlkorpgnd, %controlkong, %controlkongnorm, %controlkonggnd, %controlsub, %controlsubnorm, %controlsubgnd, %controlgeo, %controlgeonorm, %controlgeognd, %controlgenre, %nebenper, %nebenpernorm, %nebenpergnd, %nebenperrelator, %nebenkorp, %nebenkorpnorm, %nebenkorpgnd, %nebenkorprelator, %nebenkong, %nebenkongnorm, %nebenkonggnd, %nebenkongrelator, %originationa, %originationgnd,  %hyperlink, %hyperlinknote, %hide, %catdate, %catdatehuman );
...
$importer->each(sub {
    ...
    if (marc_map($data,'100') ne "") {
        ...
        unshift @nebenpert, $empty;
        unshift @nebenpern, $empty;
        unshift @nebenperp, $empty;
        unshift @nebenperm, $empty;
        unshift @nebenperr, $empty;
        unshift @nebenpers, $empty;
        unshift @nebenpero, $empty;
        unshift @nebenperh, $empty;
	...
	for my $i (0 .. ($alttitle_max)-1) {
        ...
        my @content;
        my $content_max = maxarray(\@contenta, \@contentn, \@contentg, \@contentt, \@contentr, \@contenti, \@contents, \@contentv);
        ...
        my @contentform;
        my $contentform_max = maxarray(\@contentforma, \@contentformb, \@contentform3);
        ...
        my @acqinfo;
        my $acqinfo_max = maxarray(\@acqinfo3, \@acqinfoc, \@acqinfoa, \@acqinfod, \@acqinfoe, \@acqinfof);
	...
	my @literatur;
        my $literatur_max = maxarray(\@literaturi, \@literatura, \@literatur3);
	...
	my $datenorm_max = maxarray(\@datenorma, \@datenormb, \@datenormc, \@datenormd, \@datenorme);
	...
	for my $i (0 .. ($datenorm_max)-1) {
		...
		$datenormbhuman[$i] = "v" . substr($datenormbhuman[$i],4,2) . "." . substr($datenormbhuman[$i],0,4);
	        $datenormb[$i] = "-" . substr($datenormb[$i],0,4) . "-" . substr($datenormb[$i],4,2)
	    } elsif (length($datenormb[$i]) == 8) {
	        $datenormbhuman[$i] = "v" . substr($datenormbhuman[$i],6,2) . "." . substr($datenormbhuman[$i],4,2) . "." . substr($datenormbhuman[$i],0,4);
	    ...
	    if (length($datenormc[$i]) == 6) {
	        $datenormchuman[$i] = substr($datenormchuman[$i],4,2) . "." . substr($datenormchuman[$i],0,4);
	        $datenormc[$i] = substr($datenormc[$i],0,4) . "-" . substr($datenormc[$i],4,2)
	    } elsif (length($datenormc[$i]) == 8) {
	        $datenormchuman[$i] = substr($datenormchuman[$i],6,2) . "." . substr($datenormchuman[$i],4,2) . "." . substr($datenormchuman[$i],0,4)
	    }
	    
	    if (length($datenormd[$i]) == 6) {
                $datenormdhuman[$i] = "v" . substr($datenormdhuman[$i],4,2) . "." . substr($datenormdhuman[$i],0,4);
	        $datenormd[$i] = "-" . substr($datenormd[$i],0,4) . "-" . substr($datenormd[$i],4,2)
	    } elsif (length($datenormd[$i]) == 8) {
                $datenormdhuman[$i] = "v" . substr($datenormdhuman[$i],6,2) . "." . substr($datenormdhuman[$i],4,2) . "." . substr($datenormdhuman[$i],0,4);
	    ...
	    if (length($datenorme[$i]) == 6) {
   	        $datenormehuman[$i] =  substr($datenormehuman[$i],4,2) . "." . substr($datenormehuman[$i],0,4);
	        $datenorme[$i] = substr($datenorme[$i],0,4) . "-" . substr($datenorme[$i],4,2)
	    } elsif (length($datenorme[$i]) == 8) {
	        $datenormehuman[$i] =  substr($datenorme[$i],6,2) . "." . substr($datenormehuman[$i],4,2) . "." . substr($datenormehuman[$i],0,4)
	    }

	    $datenormc[$i] = $datenormb[$i] unless $datenormc[$i];
	    $datenorme[$i] = $datenormd[$i] unless $datenorme[$i];
	    
	    $datenormchuman[$i] = $datenormbhuman[$i] unless $datenormchuman[$i];
	    $datenormehuman[$i] = $datenormdhuman[$i] unless $datenormehuman[$i];

        my $newdatenormhuman;

        $newdatenormhuman = $datenormchuman[$i] . "-" . $datenormehuman[$i] unless $datenormehuman[$i] eq "";

        push @datenormhuman, $newdatenormhuman;

        my $newdatenorm;

        $newdatenorm = $datenormc[$i] . "/" . $datenorme[$i] unless $datenorme[$i] eq "";

	    if ($newdatenorm =~ /^(-?(0|1|2)([0-9]{3})(((01|02|03|04|05|06|07|08|09|10|11|12)((0[1-9])|((1|2)[0-9])|(3[0-1])))|-((01|02|03|04|05|06|07|08|09|10|11|12)(\-((0[1-9])|((1|2)[0-9])|(3[0-1])))?))?)(\/-?(0|1|2)([0-9]{3})(((01|02|03|04|05|06|07|08|09|10|11|12)((0[1-9])|((1|2)[0-9])|(3[0-1])))|-((01|02|03|04|05|06|07|08|09|10|11|12)(-((0[1-9])|((1|2)[0-9])|(3[0-1])))?))?)?$/) {
	...
	my @controlper;
	my $controlper_max = maxarray(\@controlpera, \@controlperq, \@controlperb, \@controlperc, \@controlperd);
	...
	my @controlsub;
	my $controlsub_max = maxarray(\@controlsuba, \@controlsubv, \@controlsubx, \@controlsuby, \@controlsubz);
	...
	my @controlgeo;
	my $controlgeo_max = maxarray(\@controlgeoa, \@controlgeov, \@controlgeox, \@controlgeoy, \@controlgeoz);
	...
	my @nebenper;
	my $nebenper_max = maxarray(\@nebenpera, \@nebenperq, \@nebenperb, \@nebenperc, \@nebenperd, \@nebenpert, \@nebenpern, \@nebenperp, \@nebenperm, \@nebenperr, \@nebenpers, \@nebenpero, \@nebenperh);
	...
	if ($hide =~ /hide_this ead/) {
        ...
        unless (($hide =~ /hide_this/) || ( $level =~ /Abteilung/) || ( $level =~ /Hauptabteilung/) || ($hide =~ /collect_this.handschrift/ && $institution =~ /UBHandschriften/) || ($hide =~ /collect_this.miszellan/ && $institution =~ /UBHandschriften/) || ($hide =~ /collect_this.handschrift/ && $institution =~ /Luzern.ZHB/) || ($hide =~ /collect_this.handschrift/ && $institution =~ /Vadiana/) || ( $institution =~ /REBUS/)) {
...
sub intro{
    ...
    $writer->startTag("eadheader", "langencoding" => "iso639-2b", "scriptencoding" => "iso15924", "dateencoding" => "iso8601", "countryencoding" => "iso3166-1", "repositoryencoding" => "iso15511", "relatedencoding" => "Marc21", "audience" => "external");
    ...
    if ( $institution{$sysnum} =~ /Rorschach/ ) {
        $writer->startTag("language", "scriptcode" => "Latn", "langcode" => "eng"); 
        $writer->characters("Englisch"); 
    } else {
        $writer->startTag("language", "scriptcode" => "Latn", "langcode" => "ger"); 
...
sub ead{
   ...
   $writer->startTag($lvl{$level{$sysnum}}, "level" => $lvlarg{$level{$sysnum}}, "id" => $isilsysnum{$sysnum});
   $writer->startTag("did");
   ...
   foreach my $i (0 .. (@{$hyperlink{$sysnum}}-1)) {
       $writer->startTag("dao", [$xlink, "href"] => $hyperlink{$sysnum}[$i], [$xlink, "title"] => $hyperlinknote{$sysnum}[$i]);
       $writer->endTag("dao");
   }

   unless ($hide{$sysnum} =~ /einzel/) {
       $writer->startTag("dao", [$xlink, "type"] => "simple", [$xlink, "show"] => "embed", [$xlink, "actuate"] => "onLoad", [$xlink, "href"] => 'http://aleph.unibas.ch/F/?local_base=DSV05&con_lng=GER&func=find-b&find_code=SYS&request=' . $sysnum, [$xlink, "title"] => "Katalogeintrag im Verbundkatalog HAN");
       $writer->endTag("dao");
   }
		                
   $writer->startTag("repository");
   $writer->startTag("corpname", "role" => "Bestandshaltende Einrichtung", "normal" => $institutionname{$sysnum}[0], "authfilenumber" =>  $isilnum{$sysnum});
   ...
   foreach my $i (0 .. (@{$nebenper{$sysnum}}-1)) {
       if ($nebenperrelator{$sysnum}[$i] eq 'Aktenbildner') {
           if ($nebenpergnd{$sysnum}[$i] ne "") {
               ...
               $writer->startTag("persname", "normal" => $nebenpernorm{$sysnum}[$i], "role" => "Bestandsbildner",  "source" => "GND", "authfilenumber" => "$nebenpergnd{$sysnum}[$i]");
               $writer->characters($nebenper{$sysnum}[$i]);
               $writer->endTag("persname");
               $writer->endTag("origination");
           } else {
               $writer->startTag("origination");
               $writer->startTag("persname", "normal" => $nebenpernorm{$sysnum}[$i], "role" => "Bestandsbildner");
               $writer->characters($nebenper{$sysnum}[$i]);
               $writer->endTag("persname");
               $writer->endTag("origination");
           }
   ...
   foreach my $i (0 .. (@{$nebenkorp{$sysnum}}-1)) {
       if ($nebenkorprelator{$sysnum}[$i] eq 'Aktenbildner') {
           if ($nebenkorpgnd{$sysnum}[$i] ne "") {
               $writer->startTag("origination");
               $writer->startTag("corpname","normal" => $nebenkorpnorm{$sysnum}[$i], "role" => "Bestandsbildner", "source" => "GND", "authfilenumber" => "$nebenkorpgnd{$sysnum}[$i]");
               $writer->characters($nebenkorp{$sysnum}[$i]);
               $writer->endTag("corpname");
               $writer->endTag("origination");
           } else {
               $writer->startTag("origination");
               $writer->startTag("corpname","normal" => $nebenkorpnorm{$sysnum}[$i], "role" => "Bestandsbildner");
               $writer->characters($nebenkorp{$sysnum}[$i]);
               $writer->endTag("corpname");
               $writer->endTag("origination");
           }
   ...
   foreach my $i (0 .. (@{$nebenkong{$sysnum}}-1)) {
       if ($nebenkongrelator{$sysnum}[$i] eq 'Aktenbildner') {
           if ($nebenkonggnd{$sysnum}[$i] ne "") {
               $writer->startTag("origination");
               $writer->startTag("corpname","normal" => $nebenkongnorm{$sysnum}[$i], "role" => "Bestandsbildner", "source" => "GND", "authfilenumber" => "$nebenkonggnd{$sysnum}[$i]");
               $writer->characters($nebenkong{$sysnum}[$i]);
               $writer->endTag("corpname");
               $writer->endTag("origination");
           } else {
               $writer->startTag("origination");
               $writer->startTag("corpname","normal" => $nebenkongnorm{$sysnum}[$i], "role" => "Bestandsbildner");
               $writer->characters($nebenkong{$sysnum}[$i]);
               $writer->endTag("corpname");
               $writer->endTag("origination");
           }
   ...
   if (hasvalue($title{$sysnum})) {
       ...
       foreach my $i (0 .. (@{$alttitle{$sysnum}}-1)) {
           $writer->startTag("unittitle", "label" => $alttitleintro{$sysnum}[$i]);
   ...
   } elsif (hasvalue($datenormhuman{$sysnum}[0]) && @{$datenormhuman{$sysnum}} == 1) { 
   ...
   foreach my $i (0 .. (@{$note{$sysnum}}-1)) {
       $writer->startTag("note","label" => "Bemerkung", "audience" => "external");
       $writer->startTag("p");
   ...
   if ((@{$controlper{$sysnum}} > 0) || ( @{$nebenper{$sysnum}} > 0 && !('Aktenbildner' ~~ @{$nebenperrelator{$sysnum}}))) {
       ...
       foreach my $i (0 .. (@{$controlper{$sysnum}}-1)) {
           if ($controlperfamily{$sysnum}[$i] =~ /Familie/) {
               if ($controlpergnd{$sysnum}[$i] ne "") {
                   $writer->startTag("persname","role" => "Erwähnte Familie", "normal" => $controlpernorm{$sysnum}[$i], "source" => "GND", "authfilenumber" => "$controlpergnd{$sysnum}[$i]");
                   $writer->characters($controlper{$sysnum}[$i]);
                   $writer->endTag("persname");
               } else {
                   $writer->startTag("persname","role" => "Erwähnte Familie", "normal" => $controlpernorm{$sysnum}[$i]);
                   $writer->characters($controlper{$sysnum}[$i]);
                   $writer->endTag("persname");
               }
           } else {
               if ($controlpergnd{$sysnum}[$i] ne "") {
                   $writer->startTag("persname","role" => "Erwähnte Person", "normal" => $controlpernorm{$sysnum}[$i], "source" => "GND", "authfilenumber" => "$controlpergnd{$sysnum}[$i]");
                   $writer->characters($controlper{$sysnum}[$i]);
                   $writer->endTag("persname");
               } else {
                   $writer->startTag("persname","role" => "Erwähnte Person", "normal" => $controlpernorm{$sysnum}[$i]);
                   $writer->characters($controlper{$sysnum}[$i]);
                   $writer->endTag("persname");
               }
           } 
       ...
       foreach my $i (0 .. (@{$nebenper{$sysnum}}-1)) {
           unless ($nebenperrelator{$sysnum}[$i] eq 'Aktenbildner') {
               if ($nebenpergnd{$sysnum}[$i] ne "") {
                   $writer->startTag("persname", "normal" => $nebenpernorm{$sysnum}[$i], "source" => "GND", "authfilenumber" => "$nebenpergnd{$sysnum}[$i]", "role" => $nebenperrelator{$sysnum}[$i]);
                   $writer->characters($nebenper{$sysnum}[$i]);
                   $writer->endTag("persname");
               } else {
                   $writer->startTag("persname", "normal" => $nebenpernorm{$sysnum}[$i], "role" => $nebenperrelator{$sysnum}[$i]);
                   $writer->characters($nebenper{$sysnum}[$i]);
                   $writer->endTag("persname");
               }
           }
   ...
   if ((@{$controlkorp{$sysnum}} > 0) || (@{$controlkong{$sysnum}} > 0) || (@{$nebenkorp{$sysnum}} > 0 && !('Aktenbildner' ~~ @{$nebenkorprelator{$sysnum}})) || (@{$nebenkong{$sysnum}} > 0 && !('Aktenbildner' ~~ @{$nebenkongrelator{$sysnum}})))  {
      $writer->startTag("controlaccess");
      ...
      foreach my $i (0 .. (@{$controlkorp{$sysnum}}-1)) {
          if ($controlkorpgnd{$sysnum}[$i] ne "") {
              $writer->startTag("corpname","role" => "Erwähnte Körperschaft", "normal" => $controlkorpnorm{$sysnum}[$i], "source" => "GND", "authfilenumber" => "$controlkorpgnd{$sysnum}[$i]");
              $writer->characters($controlkorp{$sysnum}[$i]);
              $writer->endTag("corpname");
          } else {
              $writer->startTag("corpname","role" => "Erwähnte Körperschaft", "normal" => $controlkorpnorm{$sysnum}[$i]);
              $writer->characters($controlkorp{$sysnum}[$i]);
              $writer->endTag("corpname");
          }
      }
      ...
      foreach my $i (0 .. (@{$nebenkorp{$sysnum}}-1)) {
          unless ($nebenkorprelator{$sysnum}[$i] eq 'Aktenbildner') {
              if ($nebenkorpgnd{$sysnum}[$i] ne "") {
                  $writer->startTag("corpname","normal" => $nebenkorpnorm{$sysnum}[$i], "source" => "GND", "authfilenumber" => "$nebenkorpgnd{$sysnum}[$i]", "role" => $nebenkorprelator{$sysnum}[$i]);
                  $writer->characters($nebenkorp{$sysnum}[$i]);
                  $writer->endTag("corpname");
              } else {
                  $writer->startTag("corpname","normal" => $nebenkorpnorm{$sysnum}[$i], "role" => $nebenkorprelator{$sysnum}[$i]);
                  $writer->characters($nebenkorp{$sysnum}[$i]);
                  $writer->endTag("corpname");
              }
          }
      ...
      foreach my $i (0 .. (@{$controlkong{$sysnum}}-1)) {
          if ($controlkonggnd{$sysnum}[$i] ne "") {
              $writer->startTag("corpname","role" => "Erwähnte Körperschaft", "normal" => $controlkongnorm{$sysnum}[$i], "source" => "GND", "authfilenumber" => "$controlkonggnd{$sysnum}[$i]");
              $writer->characters($controlkong{$sysnum}[$i]);
              $writer->endTag("corpname");
          } else { 
              $writer->startTag("corpname","role" => "Erwähnte Körperschaft", "normal" => $controlkongnorm{$sysnum}[$i]);
              $writer->characters($controlkong{$sysnum}[$i]);
              $writer->endTag("corpname");
          }
      }
	  ...
	  unless ($nebenkongrelator{$sysnum}[$i] eq 'Aktenbildner') {
              if ($nebenkonggnd{$sysnum}[$i] ne "") {
                  $writer->startTag("corpname","normal" => $nebenkongnorm{$sysnum}[$i], "source" => "GND", "authfilenumber" => "$nebenkonggnd{$sysnum}[$i]", "role" => $nebenkongrelator{$sysnum}[$i]);
                  $writer->characters($nebenkong{$sysnum}[$i]);
                  $writer->endTag("corpname");
              } else {
                  $writer->startTag("corpname","normal" => $nebenkongnorm{$sysnum}[$i], "role" => $nebenkongrelator{$sysnum}[$i]);
                  $writer->characters($nebenkong{$sysnum}[$i]);
                  $writer->endTag("corpname");
              }
	 }
   ...
   unless (@{$controlsub{$sysnum}} == 0) {
       ...
       foreach my $i (0 .. (@{$controlsub{$sysnum}}-1)) {
           if (defined $controlsubgnd{$sysnum}[$i]) {
               $writer->startTag("subject","normal" => $controlsubnorm{$sysnum}[$i], "source" => "GND", "authfilenumber" => "$controlsubgnd{$sysnum}[$i]");
               $writer->characters($controlsub{$sysnum}[$i]);
               $writer->endTag("subject");
           } else {
               $writer->startTag("subject","normal" => $controlsubnorm{$sysnum}[$i]);
               $writer->characters($controlsub{$sysnum}[$i]);
               $writer->endTag("subject");
           }
       }
   ...
   unless ((@{$controlgeo{$sysnum}} == 0) && (@{$originationa{$sysnum}} == 0)) {
       $writer->startTag("controlaccess");
       ...
       foreach my $i (0 .. (@{$controlgeo{$sysnum}}-1)) {
           if ($controlgeognd{$sysnum}[$i] ne "") {
               $writer->startTag("geogname","role" => "Erwähnter Ort", "normal" => $controlgeonorm{$sysnum}[$i], "source" => "GND", "authfilenumber" => "$controlgeognd{$sysnum}[$i]");
               $writer->characters($controlgeo{$sysnum}[$i]);
               $writer->endTag("geogname");
           } else {
               $writer->startTag("geogname","role" => "Erwähnter Ort", "normal" => $controlgeonorm{$sysnum}[$i]);
               $writer->characters($controlgeo{$sysnum}[$i]);
               $writer->endTag("geogname");
           }
       }
       ...
       foreach my $i (0 .. (@{$originationa{$sysnum}}-1)) {
           if ($originationgnd{$sysnum}[$i] ne "") {
               $writer->startTag("geogname","role" => "Entstehungsort", "normal" => $originationa{$sysnum}[$i], "source" => "GND", "authfilenumber" => "$originationgnd{$sysnum}[$i]");
               $writer->characters($originationa{$sysnum}[$i]);
               $writer->endTag("geogname");
           } else {
               $writer->startTag("geogname","role" => "Entstehungsort", "normal" => $originationa{$sysnum}[$i]);
               $writer->characters($originationa{$sysnum}[$i]);
               $writer->endTag("geogname");
           }
       }
   ...
   if (hasvalue($catdate{$sysnum}[0])) { 
       $writer->startTag("date","type" => "Erfassungsdatum","normal" => $catdate{$sysnum}[0]);
       $writer->characters($catdatehuman{$sysnum}[0]); 
       $writer->endTag("date");
   }
   
   if (hasvalue($catdate{$sysnum}[-1])) { 
       $writer->startTag("date","type" => "Modifikationsdatum","normal" => $catdate{$sysnum}[-1]);
       $writer->characters($catdatehuman{$sysnum}[-1]); 
       $writer->endTag("date");
   }
   
   $writer->startTag("date","type" => "Synchronisationsdatum","normal" => $syncdate);
